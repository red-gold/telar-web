...
func loadConfigFromEnvironment() {
	...
	if ok {
		ActionConfig.BaseRoute = base_route
	...
	if ok {
		...
		ActionConfig.QueryPrettyURL = parsedQueryPrettyURL
		log.Printf("[INFO]: Query Pretty URL information loaded from env.")
	}

	websocketURL, ok := os.LookupEnv("websocket_server_url")
	if ok {
		ActionConfig.WebsocketServerURL = websocketURL
		log.Printf("[INFO]: Websocket server URL information loaded from env.")
	}
	...
	if ok {
		...
		ActionConfig.Debug = parsedDebug
